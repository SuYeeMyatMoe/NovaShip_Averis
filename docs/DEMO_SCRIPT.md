...
| 4:10–4:40 | P3 | Audit History + architecture slide | "30 append-only events: SYSTEM, AI and USER actors, before/after, policy version. Supabase with RLS, signed document URLs, Docker images, Gmail API connector. Retries are idempotent." Show `case_email_512`: "Scanned PDF → human review with reason, not a guess." |
