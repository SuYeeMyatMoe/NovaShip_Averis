...
- [ ] Negative check: open any **seeded** case (e.g. `case_email_004`) → Approve its draft → `502 no mailbox can send this reply` and the draft shows `SEND_FAILED` (nothing left the desk). Expected: seeded cases did not arrive through your mailbox and there is no shared mailbox.
