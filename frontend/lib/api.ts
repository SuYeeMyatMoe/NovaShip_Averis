...
export type PolicySuggestions = { items: PolicySuggestion[]; total: number; progress: { recipe: string; kind?: string; label?: string; bucket: string; count: number; needed: number; blocked_by?: { case_id: string; decision: string; status: string }[]; note?: string }[]; settings: Record<string, any>; counts?: { archived_flagged: number; conflicted_senders: number }; evidence_rule?: string };
