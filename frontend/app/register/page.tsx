...
function RoleSelect({ role, roles, onChange }: { role: string; roles: string[]; onChange: (r: string) => void }) {
  return (
    <div className="relative">
      <select name="role" value={role} required disabled={!roles.length} onChange={(e) => onChange(e.target.value)} className={`${inputClass} cursor-pointer appearance-none pr-10`}>
        {roles.length !== 1 && <option value="" disabled>Select a role</option>}
