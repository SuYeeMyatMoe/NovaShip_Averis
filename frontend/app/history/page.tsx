            ...
            <tbody>
              ...
              {rows?.map((r) => (
                <tr key={r.case_id} onClick={(e) => { if ((e.target as HTMLElement).closest("button, a")) return; router.push(`/cases/${r.case_id}`); }} title="Open the case"
                  ...
                  <td className="px-2 py-2 text-[11px]">{r.run_by_name}{r.mode === "batch" && <span className="ml-1 rounded-full bg-ink-100 px-1.5 text-[10px] text-ink-600">batch</span>}{r.mode === "human" && <span className="ml-1 rounded-full bg-accent-bg px-1.5 text-[10px] text-accent-fg" title="Marked complete by a person">by hand</span>}{r.mode === "pipeline" && <span className="ml-1 rounded-full bg-review-bg px-1.5 text-[10px] text-review-fg" title="Handed to a person by the pipeline / security agent">waiting</span>}</td>
