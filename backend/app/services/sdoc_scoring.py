"""
Vendored copy of the organiser's scorer (sdoc-hackathon-docker/server/scoring.py) so the self-evaluation
works on deployments that ship only backend/ (e.g. Vercel). Keep byte-identical to the original below the
marker line; tests/test_evaluation.py fails when the two drift.
"""
# --- ORIGINAL BELOW (do not edit; re-copy from sdoc-hackathon-docker/server/scoring.py) ---
#!/usr/bin/env python3
"""
